"use client";

import { useState } from "react";
import { buildOAuthCallbackUrl } from "@/lib/auth/oauth-callback";
import { createClient } from "@/lib/supabase/client";

type GoogleOAuthButtonProps = {
  /** Post-auth path only (must start with `/`, same origin). */
  nextPath: string;
  disabled?: boolean;
  label?: string;
  /** Client callback; `Action` suffix satisfies Next.js client-entry prop checks (not a Server Action). */
  onErrorAction?: (message: string) => void;
};

export function GoogleOAuthButton({
  nextPath,
  disabled,
  label = "Continue with Google",
  onErrorAction,
}: GoogleOAuthButtonProps) {
  const [busy, setBusy] = useState(false);

  async function handleClick() {
    if (disabled || busy) return;
    setBusy(true);
    let scheduledNavigation = false;
    const startTimeoutMs = 25_000;
    try {
      const redirectTo = buildOAuthCallbackUrl(nextPath);
      if (!/^https?:\/\//i.test(redirectTo)) {
        onErrorAction?.("Set NEXT_PUBLIC_SITE_URL in .env.local to your app URL (e.g. http://localhost:3000) so Google sign-in can redirect back.");
        return;
      }
      const supabase = createClient(true);
      const oauthPromise = supabase.auth.signInWithOAuth({
        provider: "google",
        options: {
          redirectTo,
          queryParams: { prompt: "select_account" },
          // We navigate ourselves on the next task (below). Reasons:
          // - Edge often misses navigation when GoTrue calls assign() inline after await.
          // - One navigation avoids Chrome "Throttling navigation" from double assign.
          skipBrowserRedirect: true,
        },
      });
      const raced = await Promise.race([
        oauthPromise.then((r) => ({ ok: true as const, r })),
        new Promise<{ ok: false; reason: "timeout" }>((resolve) =>
          setTimeout(() => resolve({ ok: false, reason: "timeout" }), startTimeoutMs)
        ),
      ]);
      if (!raced.ok) {
        onErrorAction?.(
          "Google sign-in is taking too long. Check your connection and try again, or use email sign-in."
        );
        return;
      }
      const { data, error } = raced.r;
      if (error) {
        onErrorAction?.(error.message || "Could not start Google sign-in.");
        return;
      }
      const url = data.url;
      if (url) {
        scheduledNavigation = true;
        window.setTimeout(() => {
          window.location.assign(url);
        }, 0);
        return;
      }
      onErrorAction?.("Could not start Google sign-in.");
    } catch (e) {
      onErrorAction?.(e instanceof Error ? e.message : "Could not start Google sign-in.");
    } finally {
      if (!scheduledNavigation) {
        setBusy(false);
      }
    }
  }

  return (
    <button
      type="button"
      onClick={() => void handleClick()}
      disabled={disabled || busy}
      className="flex w-full items-center justify-center gap-3 rounded-xl border border-gray-200 bg-white py-3 sm:py-3.5 text-sm sm:text-base font-semibold text-gray-800 shadow-sm transition hover:bg-gray-50 disabled:opacity-60"
    >
      <span className="inline-flex h-5 w-5 shrink-0" aria-hidden>
        <svg viewBox="0 0 24 24" className="h-5 w-5">
          <path
            fill="#4285F4"
            d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"
          />
          <path
            fill="#34A853"
            d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"
          />
          <path
            fill="#FBBC05"
            d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"
          />
          <path
            fill="#EA4335"
            d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"
          />
        </svg>
      </span>
      {busy ? "Redirecting…" : label}
    </button>
  );
}
